...
maryland_county_demographics <- get_acs(geography = "county", state="MD",variables = c( "B01001_001","B02001_002","B02001_003","B02001_004","B03001_003","B06012_002","B19013_001"), year = 2020) %>%
...
###
# Total population, with geoid, no county names
###

maryland_county_population <- maryland_county_demographics %>%
  select(geoid,total_pop)

###
# Lookup table geoid and name
###


maryland_county_lookup_table <- maryland_county_demographics %>%
  select(geoid,name)


###
# geoid and population with no pg county
###

maryland_county_population_no_pg <- maryland_county_population %>%
  filter(geoid != 24033)
...
maryland_large_county_population <- maryland_county_demographics %>%
  select(geoid,name,total_pop) %>%
  arrange(desc(total_pop)) %>%
  head(10)

maryland_small_county_population <- maryland_county_demographics %>%
  select(geoid,name,total_pop) %>%
  arrange(total_pop) %>%
  head(14)
...
write_rds(maryland_county_population,paste0(output_path,"maryland_county_population.rds"))
write_rds(maryland_county_lookup_table,paste0(output_path,"maryland_county_lookup_table.rds"))
write_rds(maryland_county_population_no_pg,paste0(output_path,"maryland_county_population_no_pg.rds"))
write_rds(maryland_large_county_population, paste0(output_path,"maryland_large_county_population.rds"))

write_rds(maryland_small_county_population, paste0(output_path,"maryland_small_county_population.rds"))
...
maryland_county_population <- read_rds("data/maryland_county_population.rds")
...
maryland_county_lookup_table <- read_rds("data/maryland_county_lookup_table.rds")
...
maryland_county_population_no_pg <- read_rds("data/maryland_county_population_no_pg.rds")
...
# ![left join](assets/left-join.gif){width="100%"}
...
# ![right join](assets/right-join.gif){width="100%"}
...
# ![anti join](assets/anti-join.gif){width="100%"}
...
# ![left join extra](assets/left-join-extra.gif){width="100%"}
...
maryland_large_county_population <- read_rds("data/maryland_large_county_population.rds")
...
# County identified by GEOID (a 5-digit code) and name
###
maryland_small_county_population <- read_rds("data/maryland_small_county_population.rds")
