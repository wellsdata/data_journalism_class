...
[Twitter's API](https://developer.twitter.com/en/docs) lets us pull in a user's tweets for text analysis. [Spotify](https://developer.spotify.com/documentation/web-api/) let's us ingest a bunch of information about what people are listening to. 

You can access data available through pretty much any API, or application programming interface, by sending a "query" and getting back a response, using the excellent [HTTR package](https://httr.r-lib.org/). That process is a bit complicated for an introduction.
